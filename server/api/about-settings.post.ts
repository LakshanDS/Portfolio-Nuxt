// Upsert of the CmsSettings `about` column (create fills the other
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const about = JSON.parse((await readRawBody(event, "utf8")) ?? "");
    ...
    if (row) {
      await db
        .prepare(`UPDATE CmsSettings SET about = ?, updatedAt = ? WHERE id = 'default'`)
        .bind(json, now)
        .first();
    } else {
      await db
